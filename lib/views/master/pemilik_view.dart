...
import 'package:cims_app/services/master/pemilik_service.dart'; // Import Service

class PemilikView extends StatefulWidget {
  const PemilikView({super.key});

  @override
  State<PemilikView> createState() => _PemilikViewState();
}
...
class _PemilikViewState extends State<PemilikView> {
  ...
  // Gunakan ID Database (String), bukan Index List
  String? _selectedId;
  ...
  final PemilikService _service = PemilikService();
  ...
  void _fetchData() async {
    ...
    final data = await _service.getPemilik();
    setState(() {
      _dataList = data;
      _isLoading = false;
      _selectedId = null;
  ...
  void _saveData() async {
    ...
    if (_namaController.text.trim().isEmpty || _kodeController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Kode & Nama Pemilik wajib diisi!"), backgroundColor: Colors.red));
      return;
    }
    ...
    for (var item in _dataList) {
      // Skip cek jika sedang edit item yang sama
      if (_isEditing && item['id'].toString() == _selectedId) continue;

      // Cek Kode (Case Insensitive)
    ...
    if (isDuplicate) {
      showDialog(context: context, builder: (c) => AlertDialog(title: const Text("Kode Duplikat"), content: Text("Kode '$inputCode' sudah digunakan."), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text("OK"))]));
      return;
    }
    ...
    if (_isEditing && _selectedId != null) {
      ...
      success = await _service.updatePemilik(formData);
    } else {
      success = await _service.addPemilik(formData);
    }
    ...
    showDialog(context: context, builder: (c) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: vosenRed, foregroundColor: Colors.white),
              onPressed: () async {
                Navigator.pop(c);
                if (await _service.deletePemilik(_selectedId!)) {
                  _fetchData(); _resetForm();
  ...
  void _resetForm() {
    _kodeController.clear(); _namaController.clear(); _alamatController.clear(); _telpController.clear(); _ketController.clear();
    ...
    return Row(
      children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Data Pemilik", style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: vosenDarkBlue)),
          const SizedBox(height: 5),
          Text("Master Management > Pemilik", style: TextStyle(color: Colors.grey[500], fontSize: 14)),
        ]),
      ],
    );
  }
      ...
      child: Column(
        children: [
          ...
          Container(padding: const EdgeInsets.symmetric(vertical: 15, horizontal: 15), decoration: BoxDecoration(color: bgGrey, borderRadius: BorderRadius.circular(10)), child: const Row(children: [
            ...
            Expanded(flex: 3, child: Text("NAMA PEMILIK", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))),
          ...
          _isLoading ? const Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator()) : ListView.builder(
            ...
            return InkWell(onTap: () => _onRowTap(item), borderRadius: BorderRadius.circular(10), child: Container(
              ...
              child: Row(children: [
                ...
                Expanded(flex: 3, child: Text(item['name'], style: const TextStyle(fontWeight: FontWeight.w600))),
  ...
  Widget _buildFormCard() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(_isEditing ? "Edit Pemilik" : "Tambah Pemilik Baru", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: vosenDarkBlue)),
        const SizedBox(height: 20),
        Row(children: [
          // KODE INPUT MANUAL
          Expanded(flex: 1, child: _buildTextField("Kode (Contoh: PK)", _kodeController, maxLength: 3, isCode: true, isMandatory: true)),
          const SizedBox(width: 20),
          Expanded(flex: 3, child: _buildTextField("Nama Pemilik", _namaController, isMandatory: true)),
        ]),
        const SizedBox(height: 20),
        _buildTextField("No. Telepon", _telpController),
        const SizedBox(height: 20),
  ...
  Widget _buildTextField(String label, TextEditingController controller, {bool readOnly = false, int maxLines = 1, int? maxLength, bool isCode = false, bool isMandatory = false}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(
          controller: controller, readOnly: readOnly, maxLines: maxLines, maxLength: maxLength,
